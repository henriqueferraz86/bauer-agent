...
| Scheduler | `bauer cron create`, `bauer cron list`, `bauer cron run <id>`, `bauer runtime start` (supervisor always-on: dispatcher + cron + outbox + kanban; autopilot opt-in), `bauer runtime fleet start` (multi-projeto) |
