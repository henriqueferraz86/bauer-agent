...
class FactoryToolsMixin:
    ...
    def _app_factory_init(self, args: dict) -> str:
        ...
        lines.append(
            ...
            "  GATE DISCOVERY ativo — escrita de código BLOQUEADA (docs/ liberado).\n"
            "  SUA PRÓXIMA AÇÃO É UMA TOOL CALL — não responda em texto.\n"
            "  DISCOVERY = você RASCUNHA, o usuário aprova depois. NÃO interrogue.\n"
            "  Preencha os docs a partir da IDEIA cobrindo 4 dimensões:\n"
            "    (1) usuários-alvo  (2) funcionalidades da V1\n"
            "    (3) restrições de stack/prazo/orçamento  (4) critério de sucesso.\n"
            "  Para cada dimensão que a ideia JÁ responde, use o que ela diz. O que\n"
            "  faltar, ASSUMA um default razoável e marque explicitamente como\n"
            "  'Premissa (assumida)' no doc — nunca invente requisito escondido.\n"
            "  Use 'clarify' SÓ se algo ESSENCIAL estiver genuinamente ambíguo\n"
            "  (no máximo 1-2 perguntas) — NUNCA pergunte o que a ideia já respondeu.\n"
            "  O usuário revisa tudo (e as premissas) no checkpoint de planejamento\n"
            "  quando o gate virar IMPLEMENTATION. Comece por docs/SPEC.md."
        )
        return "\n".join(lines)
