...
_MAPA_COR = {
    "ACCENT": "ACCENT", "PULSE": "ACCENT_DEEP", "DIM": "DIM",
...
class ExecutionIndicator:
    ...
    def start(self, description=''):
        ...
        if self.mode == 'progress':
            self._progress = Progress(
                SpinnerColumn(spinner_name=MODERN_SPINNER, style=_style(theme.ACCENT)),
                TextColumn('{task.description}', style=_style(theme.WHITE)),
                BarColumn(bar_width=24, style=_style(theme.DIM), completed_style=_style(theme.ACCENT)),
                TextColumn('{task.percentage:>3.0f}%', style=_style(theme.ACCENT)),
                TimeElapsedColumn(), console=self.console, transient=self.transient,
            )
    ...
    def update(self, description='', advance=0):
        if self._progress and self._task_id is not None:
            ...
            if advance and self.mode == 'progress':
                self._progress.update(self._task_id, advance=advance)

    def complete(self, result='OK Concluido'):
        if self._progress and self._task_id is not None:
            self._progress.update(self._task_id, description=f'[bold {theme.OK}]{result}[/]')
        self.stop()

    def fail(self, error='X Falhou'):
        if self._progress and self._task_id is not None:
            self._progress.update(self._task_id, description=f'[bold {theme.BAD}]{error}[/]')
        self.stop()
...
def show_header(title, console=None):
    ...
    text = Text()
    text.append('  ', style='')
    text.append('>', style=_style(theme.ACCENT))
    text.append(f'  {title}', style=_style(theme.WHITE))
    console.print(text)
